...
my $query = "select CASE WHEN ctime >= $today_unix_time_6_am AND ctime <= $today_unix_time_5_pm AND kbytes > kb_softq THEN '*'
          ELSE ' '
                  END AS OQ, name, acct_type, format(kbytes*1000,0) as bytes, format(kb_softq*1000,0) as quota, format(inodes,0) as inodes, format(i_softq,0) as i_softq, CASE WHEN ctime >= $today_unix_time_6_am AND ctime <= $today_unix_time_5_pm AND inodes > i_softq THEN '*' ELSE ' ' END AS IN_OQ from quotas_sonexions where ctime >= $today_unix_time_6_am AND ctime <= $today_unix_time_5_pm AND kb_softq <> 0 AND filesystem = 'projects' AND acct_type <> 'user' AND  ((kbytes <> 0 AND kb_softq <> 0) OR (name like 'RAIN_%' OR name like 'PRAC%' OR name like 'ILL%' OR name like 'PP%' OR name like 'DD%' OR name like 'EOT%')) order by acct_type, name";
